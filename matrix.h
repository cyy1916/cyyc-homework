...
public:
	...
	matrix operator - (const matrix& A);
	matrix operator * (float c);
	matrix eletrans(int num, int n, int row1, int row2);
	matrix eletrans(int n, int k, float rok);
	matrix eletrans(int n, int row1, int row2, int opp, float rok);
	friend matrix operator * (float real, const matrix& other);
	friend std::ostream& operator << (std::ostream& os, const matrix& other);
...
matrix matrix::operator+(const matrix& other)
{
	auto **data1 = new float*[other.row];
...
#pragma omp parallel for
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < line; j++)
		{
			data1[i][j] = data[i][j] + other.data[i][j];
		}
	}
	return matrix(row, other.line, data1);
}
...
matrix matrix::eletrans(int num, int n, int row1, int row2)
{
		if (n == 1)
		{
			float * a = data[row1];
