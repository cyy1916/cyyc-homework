...
#pragma omp parallel for
	for (int n = 0; n < i; n++)
	{
		for (int p = 0; p < j; p = p + 8)
		{
			a1 = _mm256_load_ps(a[n] + p);
			for (int m = 0; m < k; m = m + 8)
			{
				b1 = _mm256_load_ps(b[p] + m);
				c1 = _mm256_add_ps(c1, _mm256_mul_ps(a1, b1));
			}
		}
		_mm256_store_ps(sum.ma[n], c1);
	}
